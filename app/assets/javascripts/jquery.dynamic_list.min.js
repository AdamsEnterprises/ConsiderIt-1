...
$j.fn.dynamicList = function(o) {
    ...
    return this.each(function () {
        ...
        function init(carousel) {
          ...
          data.$carousel
            .addClass('dynamicList');// + (data.vertical ? 'vertical' : 'horizontal'));
            //.css(data.vertical ? 'height' : 'width', data.dim);
          data.$wrapper
            .addClass('wrapper');
            //.css('overflow', 'hidden')
            //.css(data.vertical ? 'height' : 'width', data.dim-45);
          //data.$items
          ...
          */
          ...
          $(carousel).delegate('.back', 'click', function () {
            $j(this).parents('.dynamicList').find('.back, .forward').attr('disabled', true);
          ...
          $(carousel).delegate('.arrow form', 'ajax:success', function (event, response, xhr) {
            ...
            var intermediate = $j(response['points']).find('.point_list .point_in_list');

            var point_col = $j(this).parents('.points_other, .full_column'),
                points = intermediate;

            points.hide();
            point_col.find('.point_list').append( points );

            data.$items = data.$slider.find('> li');
            data.pages = Math.ceil(data.$items.length / data.items_per_page); 
            result = gotoPage(data.currentPage + 1);
            points.css('display', '');
            return result;
          });
        ...
        function refresh() {
          ...
          // resize carousel
          data.$items = $('.carousel > ul > li', data.$carousel)
        ...
        //       page_height += $j(this).outerHeight(true);
        ...
        function gotoPage(page) {
          ...
          var items_to_hide = [];
          $j.merge(items_to_hide, data.$items.slice(0, data.first - 1));
          $j.merge(items_to_hide, data.$items.slice(data.last));
          items_to_hide = $j(items_to_hide).filter(':visible');

          if (items_to_hide.length > 0) {
            $j(items_to_hide).filter(':visible').hide();
            data.$items.slice(data.first - 1, data.last).filter(':hidden').fadeIn();//css('visibility', 'visible'); 
            //});
          } else {
            data.$items.slice(data.first - 1, data.last).filter(':hidden').fadeIn();//css('visibility', 'visible'); 
          }

          data.$wrapper.parents('.dynamicList').find('.back, .forward').removeAttr('disabled'); 
