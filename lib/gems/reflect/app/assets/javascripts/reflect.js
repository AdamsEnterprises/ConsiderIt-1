  ...
  */
  enforce_contract : function () {
    ...
    if ( $j('.reflected').length == 0 ) {
      ...
      $j( '.reflected' ).append( '<span id="rf_user_id" class="hide">' + user_id + '</span>' );
    }
    ...
    */
    initialize_delegators : function() {
      ...
      $j('.rf_comment.highlight_state .rf_comment_text .sentence')
        .on('click', Reflect.handle.sentence_click);
      ...
      $j('.bullet.full_bullet')
        .on('mouseover',  Reflect.handle.bullet_mouseover)
        .on('mouseout',  Reflect.handle.bullet_mouseout);

      $j('.bullet.full_bullet .bullet_meta .delete')
        .on('click',  function( event ) { $j( this ).siblings( '.verification' ).show(); });

      $j('.bullet.full_bullet .bullet_meta .delete_nope')
        .on('click',  function( event ) { $j( this ).parents( '.verification' ).hide(); });
      ...
      $j('.bullet.connect .submit .bullet_submit:enabled')
        .on('click', Reflect.api.post_bullet);
      ...
      $j('.rate_bullet.not_anon .flag')
        .on( 'click', function(event) { Reflect.handle.bullet_flag(event); });
